...
import { apiTipoRaza, disableApiTipoRaza, enableApiTipoMascota } from "config/api";
import { enableApiTipoRaza } from "config/api";
// Data
const VerTipoRaza = () => {
  const [listData, setListData] = useState([]);

  async function getlistData() {
    const response = await axios.get(apiTipoRaza);
  ...
  const renderHeader = () => {
    return (
      <div className="flex justify-content-end">
        <Link to={"/teypeRace/form"} style={{ color: "#49bfd9" }}>
          <Button label="Crear" />
        </Link>
      </div>
    );
  ...
  async function enableFunction(rowData) {
    console.log(rowData);
    try {
      const response = await axios.patch(`${enableApiTipoRaza}${rowData}`, { low: 1 });
  ...
  async function disableFunction(rowData) {
    try {
      const response = await axios.patch(`${disableApiTipoRaza}${rowData}`, { low: 0 });
      if (response.status === 200) {
        console.log(response);
        getlistData();
      }
  ...
  return (
    <>
      <ArgonBox py={3}>
        ...
        <Card>
          <ArgonBox display="flex" justifyContent="space-between" alignItems="center" p={3}>
            <ArgonTypography variant="h6">Lista Tipo de Raza</ArgonTypography>
          </ArgonBox>
          <ArgonBox
            sx={{
              "& .MuiTableRow-root:not(:last-child)": {
          ...
          >
            <div className="card">
              ...
              >
                ...
                <Column body={actionBodyTemplate} header="Actions" style={{ width: "12%" }} />
              </DataTable>
            </div>
          </ArgonBox>
        </Card>
